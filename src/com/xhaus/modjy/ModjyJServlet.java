...
public class ModjyJServlet extends HttpServlet {
  ...
  private static final long serialVersionUID = -8388759063127468283L;
  ...
  /**
   * Read configuration 1. Both context and servlet parameters are included in the set, so that the definition of some parameters (e.g
   * python.*) can be shared between multiple WSGI servlets. 2. servlet params take precedence over context parameters
   */
  ...
  /**
   * Initialise the modjy servlet. 1. Read the configuration 2. Initialise the jython runtime 3. Setup, in relation to the J2EE servlet
   * environment 4. Create the jython-implemented servlet 5. Initialise the jython-implemented servlet
   */
  @Override
    ...
    try {
      ...
      try {
        ...
        throw new ServletException(
            "Unable to import '" + MODJY_PYTHON_CLASSNAME + "': maybe you need to set the '" + PYTHON_HOME_PARAM + "' parameter?", ix);
      }
  ...
  /**
   * Setup the modjy environment, i.e. 1. Find the location of the modjy.jar file and add it to sys.path 2. Process the WEB-INF/lib-python
   * directory, if it exists
   *
   * @param interp - The PythonInterpreter used to service requests
  ...
  /**
   * Check if the user has requested to initialise the jython installation "site-packages". The value defaults to true, i.e. load site
   * packages
   *
   * @param props - The properties from which config options are found
   */
   ...
   *
   * @param interp - The PythonInterpreter which will execute imports
   * @param systemState - The PySystemState whose path should be updated
