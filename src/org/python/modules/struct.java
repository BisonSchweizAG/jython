...
//@formatter:off
/**
 * This module performs conversions between Python values and C structs represented as Python strings. It uses <i>format
 * strings</i> (explained below) as compact descriptions of the lay-out of the C structs and the intended conversion
 ...
 * <dd>Exception raised on various occasions; argument is a string describing what is wrong.</dd>
 * <dt><b>{@code pack}</b> (<var>fmt, v1, v2, ...</var>)</dt>
 * <dd>Return a string containing the values {@code <i>v1</i>, <i>v2</i>,  ...} packed according to the given format.
 * The arguments must match the values required by the format exactly.</dd>
 * <dt><b>{@code unpack}</b> (<var>fmt, string</var>)</dt>
 * <dd>Unpack the string (presumably packed by {@code pack(<i>fmt</i>,
 *    ...)}) according to the given format. The result is a tuple even if it contains exactly one item. The string must
 * contain exactly the amount of data required by the format (i.e. {@code len(<i>string</i>)} must equal
 * {@code calcsize(<i>fmt</i>)}).</dd>
 * <dt><b>{@code calcsize}</b> (<var>fmt</var>)</dt>
 * <dd>Return the size of the struct (and hence of the string) corresponding to the given format.
 * </dl>
 *
 * Format characters have the following meaning; the conversion between C and Python values should be obvious given
 * their types:
 * 
 * <table border="1">
 ...
 *
 * <P>
 * A format character may be preceded by an integral repeat count; e.g. the format string {@code '4h'} means exactly the
 * same as {@code 'hhhh'}.
 *
 * <P>
 ...
 *
 * <P>
 * For the "{@code s}" format character, the count is interpreted as the size of the string, not a repeat count like for
 * the other format characters; e.g. {@code '10s'} means a single 10-byte string, while {@code '10c'} means 10
 * characters. For packing, the string is truncated or padded with null bytes as appropriate to make it fit. For
 * unpacking, the resulting string always has exactly the specified number of bytes. As a special case, {@code '0s'}
 * means a single, empty string (while {@code '0c'} means 0 characters).
 *
 * <P>
 * The "{@code p}" format character can be used to encode a Pascal string. The first byte is the length of the stored
 * string, with the bytes of the string following. If count is given, it is used as the total number of bytes used,
 * including the length byte. If the string passed in to {@code pack()} is too long, the stored representation is
 * truncated. If the string is too short, padding is used to ensure that exactly enough bytes are used to satisfy the
 * count.
 *
 * <P>
 * For the "{@code I}" and "{@code L}" format characters, the return value is a Python long integer.
 *
 * <P>
 * By default, C numbers are represented in the machine's native format and byte order, and properly aligned by skipping
 * pad bytes if necessary (according to the rules used by the C compiler).
 *
 * <P>
 * Alternatively, the first character of the format string can be used to indicate the byte order, size and alignment of
 ...
 *
 * <P>
 * Native byte order is big-endian or little-endian, depending on the host system (e.g. Motorola and Sun are big-endian;
 * Intel and DEC are little-endian).
 *
 * <P>
 * Native size and alignment are defined as follows: {@code short} is 2 bytes; {@code int} and {@code long} are 4 bytes;
 * {@code float} are 4 bytes and {@code double} are 8 bytes. Native byte order is chosen as big-endian.
 *
 * <P>
 * Standard size and alignment are as follows: no alignment is required for any type (so you have to use pad bytes);
 * {@code short} is 2 bytes; {@code int} and {@code long} are 4 bytes. {@code float} and {@code double} are 32-bit and
 * 64-bit IEEE floating point numbers, respectively.
 *
 * <P>
 * Note the difference between "{@code @}" and "{@code =}": both use native byte order, but the size and alignment of
 * the latter is standardized.
 *
 * <P>
 * The form "{@code !}" is available for those poor souls who claim they can't remember whether network byte order is
 * big-endian or little-endian.
 *
 * <P>
 * There is no way to indicate non-native byte order (i.e. force byte-swapping); use the appropriate choice of
 * "{@code &lt;}" or "{@code &gt;}".
 *
 * <P>
 ...
 *
 * <P>
 * Hint: to align the end of a structure to the alignment requirement of a particular type, end the format with the code
 * for that type with a repeat count of zero, e.g. the format {@code 'llh0l'} specifies two pad bytes at the end,
 * assuming longs are aligned on 4-byte boundaries. This only works when native size and alignment are in effect;
 ...
 * <p>
 *
 * The module is based on the original structmodule.c except that all mistakes and errors are my own. Original author
 * unknown.
 * 
 * 
